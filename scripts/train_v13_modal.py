"""
train_v13_modal.py — BTC 5min model v13

Strategy:
  - Local-only data (Mar-Apr 2026, 7062 markets) — no HF mixing
  - Regime shift diagnosis: v12 mixing 2025+2026 hurt performance
  - Same v9/v10 feature set — no new features
...
Data source:
  - Modal Volume 'btc-local-data':
      ...
      /btc_local/local_markets.csv       (market_id str, slot_ts, target, resolved_at)
...
app = modal.App("polymarket-btc-train-v13", image=image)
...
def train_v13():
    ...
    # ── Step 3: Load ticks row-group by row-group ──────────────────────────────
    log.info("Step 3: Loading local ticks from Modal Volume (row-group streaming)...")
    ...
    spot_ts_arr = spot_tl.index.values.astype(float)
    spot_px_arr = spot_tl["spot_price_usdt"].values.astype(float)
    del btc; gc.collect()
    log.info("  Spot series: %d points", len(spot_ts_arr))

    # ── Feature helpers ────────────────────────────────────────────────────────
    def tick_features(grp: pd.DataFrame) -> dict:
        """Same v9/v10 features as champion — no changes."""
        n = len(grp)
        if n == 0:
            return {
    ...
    log.info("Step 4: Building feature dataset...")
    ...
    # ── Step 5: Baseline WF ────────────────────────────────────────────────────
    log.info("Step 5: Baseline walk-forward (gap=5)...")
    ...
    log.info("Step 5b: Baseline WF (gap=3 comparison)...")
    ...
    log.info("Step 6: Permutation importance...")
    ...
    # ── Step 7: Optuna HPO ─────────────────────────────────────────────────────
    log.info("Step 7: Optuna HPO (%d trials)...", OPTUNA_TRIALS)
    def objective(trial):
        p = dict(
    ...
    # ── Step 8: Optimized WF ──────────────────────────────────────────────────
    log.info("Step 8: Walk-forward (optimized)...")
    ...
    # ── Step 9: Train final calibrated model ──────────────────────────────────
    log.info("Step 9: Training final calibrated model on full data...")
    ...
    model_path = Path("/tmp/btc_model_v13.pkl")
    ...
    log.info("  v13 candidate:  AUC=%.4f  Brier=%.4f  Acc=%.4f",
    ...
    promoted = False
    if should_promote:
        log.info("Promoting v13 to HF champion...")
        api = HfApi(token=HF_TOKEN)
        api.upload_file(
            path_or_fileobj=str(model_path),
            path_in_repo="champion.pkl",
            repo_id=HF_MODEL_REPO, repo_type="model",
            commit_message=(f"Champion v13: AUC={final_auc:.4f} Brier={final_brier:.4f} "
                            f"Acc={final_acc:.4f} | local-only 2026, {len(df)} samples"),
        )
        meta_out = {
            "version":       "v13",
            ...
            "champion_compared_auc": CHAMPION_AUC,
            "notes": (f"v13: local-only 2026 data ({len(df)} samples). "
                      f"Same v9/v10 features. 7-fold WF gap={ACTIVE_GAP}. "
                      f"OB hardcoded 0.0. Isotonic calibration."),
            "best_params":   final_params,
        }
        ...
        api.upload_file(
            ...
            repo_id=HF_MODEL_REPO, repo_type="model",
            commit_message=f"Champion v13 meta AUC={final_auc:.4f}",
        )
        log.info("Champion v13 promoted: https://huggingface.co/%s", HF_MODEL_REPO)
        promoted = True
    else:
        log.warning("v13 not promoted.")
...
def main():
    print("Submitting BTC v13 training job to Modal (local-only 2026 data)...")
    r = train_v13.remote()
    print(f"\n{'='*55}")
    print("TRAINING COMPLETE — v13 (local-only 2026)")
