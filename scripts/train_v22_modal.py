"""
train_v22_modal.py — BTC 5min model v22 (A/B: OBS window alignment)
====================================================================
Key changes vs v21:
  - ROOT CAUSE FIX: v21 was trained with ticks from 0-180s but live only
    receives ticks from 0-~60s (data-api lag ~120s). This train-live mismatch
    caused all DATA_GATE skips and bad predictions when data was incomplete.
  
  - A/B TEST: trains two model variants side by side:
    A) OBS_SECS=60  — match what live actually sees. 2x30s sub-windows.
    B) OBS_SECS=180 — full window, for use with delayed entry (t=300+).
    
  - EXPANDED DATASET: merged ticks from original + pmdata expansion
  - Same Optuna tuning, walk-forward validation, and promotion gate
...
app = modal.App("btc-v22-run", image=image)
...
def train_v22():
    ...
    VARIANTS_OBS = {
        "v22_60s":  60,   # Match live data availability (data-api lag ~120s)
        "v22_180s": 180,  # Full window (needs delayed entry at t=300+)
    }
    ...
    for variant_name, obs_secs in VARIANTS_OBS.items():
        ...
        for rank_i, row in markets.iterrows():
            ...
            if n > 0:
                ...
                # Momentum: use available windows (not always 6)
                if n_windows >= 4:
                    half = n_windows // 2
    ...
    # ── Save & promote ───────────────────────────────────────────────────
    version_tag = f"v22-{best_variant}"
    ...
    if best_score < 2:
        ...
        meta = {
            ...
            "changes": (
                f"A/B test: trained with OBS_SECS={obs_secs} to match live data availability. "
                f"Expanded dataset. "
                f"Root cause fix: v21 used 180s ticks but live only had ~60s due to data-api lag."
            ),
            "ab_results": {
                vname: {
                    "n_feats": r["n_feats"],
