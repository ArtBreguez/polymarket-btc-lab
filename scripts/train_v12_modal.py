...
Lessons from all versions:
  Walk-forward with 5 folds + gap=5 is conservative with 616 samples.
  Testing: 7 folds (more robust estimate, smaller test sets)
  Testing: gap=3 (less conservative, might recover AUC)
  Testing: gap=5 vs gap=3 — run both, keep best walk-forward AUC.
  Best Brier was v6 (simpler model). v9 27-feature model is the sweet spot.
  Isotonic calibration is better than sigmoid for this dataset size.

New in v12:
  - Walk-forward: 7 folds (was 5)
  - Gap testing: run WF with gap=3 AND gap=5, use whichever gives better AUC
  - Calibration: isotonic
  - Pruning: imp_mean > 0.0005
  - min_child_samples Optuna: 5-40
  - All v9 features (no new features — isolate the WF effect)
  - Optuna: 150 trials
"""
...
def train_v12():
    ...
    # ── Feature helpers ────────────────────────────────────────────────────────
    def tick_features_v12(grp: pd.DataFrame) -> dict:
        """
    ...
    for i, row in markets_sorted.iterrows():
        ...
        feat.update(tick_features_v12(grp))
    ...
    # ── Gap comparison: run WF with gap=3 to see if it improves ──────────────
    log.info("Step 7b: Baseline WF with gap=3 (comparison)...")
    wf_gap3 = walk_forward_purged(df, features, gap=3, n_splits=7)
    log.info("  Gap=3 WF AUC: %.4f | Gap=5 WF AUC: %.4f",
             wf_gap3["wf_auc"], wf_base["wf_auc"])
    if wf_gap3["wf_auc"] > wf_base["wf_auc"]:
        log.info("  Gap=3 is better — using gap=3 for HPO")
        ACTIVE_GAP = 3
        wf_base = wf_gap3
    else:
        log.info("  Gap=5 is better — keeping gap=5")
        ACTIVE_GAP = WF_GAP
    ...
    if len(champ_feats) >= 10:
        wf_champ      = walk_forward_purged(df, champ_feats, gap=ACTIVE_GAP, n_splits=7)
    ...
    if should_promote:
        ...
        api.upload_file(
            ...
            commit_message=(f"Champion v12: AUC={final_auc:.4f} Brier={final_brier:.4f} "
                            f"Acc={final_acc:.4f} | 7-fold WF + gap comparison (3 vs 5)"),
        )
        meta_out = {
            "version":       "v12",
            ...
            "champion_compared_auc": fair_champ_auc,
            "notes": "v12: 7-fold WF, gap comparison 3vs5, isotonic, imp>0.0005, min_child 5-40. No new features — isolates WF effect.",
            "best_params":   final_params,
        }
