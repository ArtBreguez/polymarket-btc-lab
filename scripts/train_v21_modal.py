"""
train_v21_modal.py — BTC 5min model v21 (Feature Pruning)
==========================================================
Key changes vs v19:
  - ABLATION STUDY: tests 40, 35, and 30 feature variants
  - Removes low-importance features that add noise/overfit risk:
    * ob_total_depth (0.7% imp, absolute value leak)
    * btc_up_ratio_zscore_5s/20s (1.0% each, noisy, needs warm history)
    * btc_pre_1h_4h_ratio (1.2%, cold buffer issues)
    * btc_up_w0 (1.2%, earliest window = most noise)
    * prev_slot_up_ratio_4 (1.2%, 4 slots back = noise)
    * btc_dist_1k (1.2%, weak round-number signal)
    * hour_x_tw_ur (1.2%, interaction overfit risk)
    * ob_imb_w1 (1.3%, INTERPOLATED not measured in live)
    * hour_cos (1.4%, temporal overfit risk)
  - Picks the BEST variant that beats champion (2/3 gate)
...
app = modal.App("btc-v21-run", image=image)
...
def train_v21():
    ...
    # ── Step 7: Feature selection with ABLATION STUDY ──────────────────────
    ...
    all_40       = [FEATURE_COLS[i] for i in feat_rank[:40]]
    log.info("Top 40 features: %s", all_40[:15])

    # Features to prune (low importance + data quality concerns in live)
    PRUNE_5 = {
        "ob_total_depth",           # 0.7% - absolute value, leaks
        "btc_up_ratio_zscore_5s",   # 1.0% - noisy short zscore
        "btc_up_ratio_zscore_20s",  # 1.0% - needs warm history
        "btc_pre_1h_4h_ratio",      # 1.2% - cold buffer issues
        "btc_up_w0",                # 1.2% - earliest window = noise
    }
    PRUNE_10 = PRUNE_5 | {
        "prev_slot_up_ratio_4",     # 1.2% - 4 slots back, noise
        "btc_dist_1k",              # 1.2% - weak signal
        "hour_x_tw_ur",             # 1.2% - overfit risk
        "ob_imb_w1",                # 1.3% - INTERPOLATED in live
        "hour_cos",                 # 1.4% - temporal overfit
    }

    variants = {
        "v21_40feat": [f for f in all_40],
        "v21_35feat": [f for f in all_40 if f not in PRUNE_5],
        "v21_30feat": [f for f in all_40 if f not in PRUNE_10],
    }

    log.info("Ablation study: testing %d variants", len(variants))
    for name, feats in variants.items():
        log.info("  %s: %d features", name, len(feats))

    # ── Step 8: Optuna tuning (on the 40-feature baseline) ──────────────
    log.info("Step 8: Optuna tuning (%d trials) on 40-feature set...", OPTUNA_TRIALS)
    X_40 = df[all_40].values.astype(np.float32)
    ...
    def objective(trial):
        ...
        aucs = []
        for tr_idx, val_idx in TimeSeriesSplit(n_splits=N_SPLITS, gap=WF_GAP).split(X_40):
            m = lgb.LGBMClassifier(**params)
            m.fit(X_40[tr_idx], y[tr_idx])
            p = m.predict_proba(X_40[val_idx])[:, 1]
    ...
    # ── Step 9: Walk-forward evaluation — ALL VARIANTS ─────────────────
    log.info("Step 9: Walk-forward evaluation for all variants...")

    results = {}
    for vname, vfeats in variants.items():
        X_v = df[vfeats].values.astype(np.float32)
        wf_aucs, wf_briers, wf_accs = [], [], []

        for fold, (tr_idx, val_idx) in enumerate(
            TimeSeriesSplit(n_splits=N_SPLITS, gap=WF_GAP).split(X_v)
        ):
            base = lgb.LGBMClassifier(**best_params)
            cal  = CalibratedClassifierCV(base, cv=3, method="isotonic")
            cal.fit(X_v[tr_idx], y[tr_idx])
            p = cal.predict_proba(X_v[val_idx])[:, 1]
        ...
        results[vname] = r
        log.info("  %s (%d feat): AUC=%.4f Brier=%.4f Acc=%.4f",
                 vname, len(vfeats), r["wf_auc"], r["wf_brier"], r["wf_acc"])

    # ── Step 10: Pick best variant and promotion gate ──────────────────
    log.info("Step 10: Variant comparison and promotion gate")
    log.info("Champion (%s): AUC=%.4f Brier=%.4f Acc=%.4f",
             champion["version"], champion["wf_auc"], champion["wf_brier"], champion["wf_acc"])

    # Score each variant vs champion
    best_variant = None
    best_score = -1
    for vname, r in results.items():
        ...
        log.info("  %s vs champion: AUC %s (%.4f) | Brier %s (%.4f) | Acc %s (%.4f) → %d/3",
                 vname,
                 "✓" if beats_auc else "✗", r["wf_auc"],
                 "✓" if beats_brier else "✗", r["wf_brier"],
                 "✓" if beats_acc else "✗", r["wf_acc"],
                 score)
        # Prefer fewer features when scores are equal
        if score > best_score or (score == best_score and best_variant and r["n_feats"] < results[best_variant]["n_feats"]):
            best_score = score
            best_variant = vname

    log.info("Best variant: %s (%d feat, %d/3 gate)",
             best_variant, results[best_variant]["n_feats"], best_score)

    top_features = results[best_variant]["features"]
    wf_auc   = results[best_variant]["wf_auc"]
    wf_brier = results[best_variant]["wf_brier"]
    wf_acc   = results[best_variant]["wf_acc"]
    ...
    if best_score < 2:
        log.info("NOT PROMOTED (%d/3). Best variant: %s. Training complete.", best_score, best_variant)
    else:
        log.info("PROMOTING v21 (%s)! (%d/3 metrics beat champion)", best_variant, best_score)
        import tempfile
        from huggingface_hub import HfApi

        model_data = {
            "version":  f"v21-{best_variant}",
        ...
        meta = {
            ...
            "changes": (
                f"Feature pruning: {best_variant} selected from ablation study "
                f"(40/35/30 features). Removed low-importance features that "
                f"had live data quality issues or overfit risk."
            ),
            "ablation_results": {
                vname: {
                    "n_feats": r["n_feats"],
        ...
        log.info("v21 promoted to HF! AUC=%.4f Brier=%.4f Acc=%.4f (%d features)",
                 wf_auc, wf_brier, wf_acc, len(top_features))

    log.info("v21 training complete.")
