"""
train_v10_modal.py — BTC 5min model v10

Lessons from v9:
  Sigmoid calibration hurt Brier (0.1809 vs v6 best 0.1562). Back to isotonic.
  27 features (imp>0.001) is right sizing. Keep same pruning threshold.
  v6 had best Brier with simpler features — interaction terms may help.
  min_child_samples search range was 15-80, try going lower (5-40).

New in v10:
  - Calibration: back to isotonic (sigmoid was worse)
  - Optuna: min_child_samples range 5-40 (was 15-80, allow deeper trees with 27 feats)
  - 2 interaction features:
      btc_signal_conviction  = btc_up_ratio * (1 - btc_up_ratio_stability) 
                               (strong signal + consistent across windows)
      btc_momentum_vol_sync  = btc_momentum * btc_vol_accel
                               (momentum confirmed by accelerating volume)
  - Feature pruning: imp_mean > 0.0005 (slightly looser than v9's 0.001)
  - Optuna trials: 150 (keep same)
"""
...
app = modal.App("polymarket-btc-train-v10", image=image)
...
def train_v10():
    ...
    # ── Feature helpers ────────────────────────────────────────────────────────
    def tick_features_v10(grp: pd.DataFrame) -> dict:
        """
        v10 tick features — extends v9 with:
          btc_signal_conviction  — strong directional signal + consistent across windows
          btc_momentum_vol_sync  — momentum confirmed by volume acceleration
        """
        n = len(grp)
        if n == 0:
            return {
        ...
        # Strong directional signal + consistent across windows
        signal_conviction  = float((vol_up / total) * (1.0 - up_ratio_stability))
        # Momentum confirmed by volume acceleration
    ...
    for i, row in markets_sorted.iterrows():
        ...
        feat.update(tick_features_v10(grp))
    ...
    model_path = Path("/tmp/btc_model_v10.pkl")
    ...
    log.info("  v10 candidate:            AUC=%.4f  Brier=%.4f  Acc=%.4f",
    ...
    promoted = False
    if should_promote:
        log.info("Promoting v10 to HF champion...")
        api = HfApi(token=HF_TOKEN)
        api.upload_file(
            path_or_fileobj=str(model_path),
            path_in_repo="champion.pkl",
            repo_id=HF_MODEL_REPO, repo_type="model",
            commit_message=(f"Champion v10: AUC={final_auc:.4f} Brier={final_brier:.4f} "
                            f"Acc={final_acc:.4f} | isotonic calib + interaction features + looser pruning"),
        )
        meta_out = {
            "version":       "v10",
            ...
            "champion_compared_auc": fair_champ_auc,
            "notes": ("v10: isotonic calibration (back from sigmoid), imp>0.0005, min_child 5-40, "
                      "+ btc_signal_conviction, btc_momentum_vol_sync interactions"),
            "best_params":   final_params,
        }
        ...
        api.upload_file(
            ...
            repo_id=HF_MODEL_REPO, repo_type="model",
            commit_message=f"Champion v10 meta AUC={final_auc:.4f}",
        )
        log.info("Champion v10 promoted: https://huggingface.co/%s", HF_MODEL_REPO)
        promoted = True
    else:
        log.warning("v10 not promoted.")
...
def main():
    print("Submitting BTC v10 training job to Modal...")
    r = train_v10.remote()
    print(f"\n{'='*55}")
    print("TRAINING COMPLETE — v10")
