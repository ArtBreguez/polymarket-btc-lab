"""
train_v25_modal.py — BTC 5min model v25 (CLOB REAL-TIME FEATURES)
====================================================================
Key changes vs v23:
  - CLOB MICROSTRUCTURE FEATURES: Uses raw orderbook events (book + price_change)
    from BrockMisner/polymarket-btc-updown to compute 10 real-time features that
    the live bot gets with ZERO lag from the CLOB WebSocket stream.
    
  - 50 TOTAL FEATURES: 40 from v23 + 10 new clob_* features:
    clob_imb_mean, clob_imb_std, clob_imb_drift, clob_spread_mean,
    clob_spread_trend, clob_mid_velocity, clob_mid_volatility,
    clob_activity_rate, clob_depth_trend, clob_ask_pressure
    
  - SAME OBS_SECS=60: Consistent with live data availability.
    CLOB features use a 60s window of book events.
    
  - TIMING MATCH: Computes CLOB features at t=obs_secs (same moment the
    live bot would compute them), using events from [0, obs_secs).
...
Data sources (HuggingFace):
  ...
  BrockMisner/polymarket-btc-updown:
    data/orderbook/crypto=BTC/timeframe=5-minute/part-0.parquet — RAW OB events (3.3GB)
"""
import modal
...
# Use a volume for caching the large orderbook file (3.3GB)
vol = modal.Volume.from_name("btc-training-cache", create_if_missing=True)

app = modal.App("btc-v25-run", image=image)
...
def train_v25():
    ...
    HF_OB_DATASET = "BrockMisner/polymarket-btc-updown"
    OB_RAW_FILE   = "data/orderbook/crypto=BTC/timeframe=5-minute/part-0.parquet"
    SLOT_DURATION = 300
    OPTUNA_TRIALS = 150
    WF_GAP        = 5
    N_SPLITS      = 5
    TOP_N_FEATS   = 50  # v25: 40 base + 10 CLOB features
    DATA_DIR      = Path("/tmp/btc_data")
    DATA_DIR.mkdir(exist_ok=True)
    CACHE_DIR     = Path("/cache")  # Modal volume for large files

    # Single variant — v25 is always 60s (matching live)
    VARIANTS_OBS = {
        "v25_60s":  60,
    }
    ...
    # ── Step 3b: Load RAW orderbook events for CLOB features ──────────────
    log.info("Step 3b: Loading raw orderbook events (3.3GB)...")
    ob_raw_cache = CACHE_DIR / "ob_raw_btc_5min.parquet"
    if ob_raw_cache.exists():
        log.info("  Using cached raw OB from volume (%.1f GB)", ob_raw_cache.stat().st_size / 1e9)
        ob_raw = pd.read_parquet(str(ob_raw_cache))
    else:
        log.info("  Downloading from %s ...", HF_OB_DATASET)
        t0 = time.time()
        dl_path = hf_hub_download(
            repo_id=HF_OB_DATASET, filename=OB_RAW_FILE,
            repo_type="dataset", token=HF_TOKEN,
            local_dir=str(DATA_DIR), local_dir_use_symlinks=False,
        )
        ob_raw = pd.read_parquet(dl_path)
        log.info("  Downloaded (%.1fs, %.1f GB, %d rows)",
                 time.time() - t0, os.path.getsize(dl_path) / 1e9, len(ob_raw))
    ...
    # - Feed all book/price_change events in [0, obs_secs) chronologically
    # - Compute features at t=obs_secs (same moment live bot computes them)
    log.info("Step 3c: Computing CLOB features per market...")

    CLOB_FEATURE_NAMES = [
        "clob_imb_mean", "clob_imb_std", "clob_imb_drift",
        "clob_spread_mean", "clob_spread_trend",
        "clob_mid_velocity", "clob_mid_volatility",
        "clob_activity_rate", "clob_depth_trend", "clob_ask_pressure",
    ]

    def _compute_clob_features_for_slot(events_df, obs_secs=60):
        """Simulate ClobFeatureAccumulator on historical events.
        
        events_df: DataFrame with columns [t_sec, event_type, asks, bids, price, side]
                   filtered to [0, obs_secs) for one market.
        Returns dict of 10 CLOB features.
        """
        zeros = {k: 0.0 for k in CLOB_FEATURE_NAMES}
        
        # Parse book events into snapshots
        book_events = events_df[events_df["event_type"] == "book"]
        pc_events = events_df[events_df["event_type"] == "price_change"]
        
        # Extract book snapshots
        snapshots = []  # list of (t_sec, best_ask, best_bid, mid, spread, imbalance, total_depth)
        for _, row in book_events.iterrows():
            try:
                ...
                if isinstance(asks[0], dict):
                    ...
                    ask_sz = float(asks[0].get("size", asks[0].get("s", 0)))
                    best_bid = float(bids[0].get("price", bids[0].get("p", 0)))
                    bid_sz = float(bids[0].get("size", bids[0].get("s", 0)))
                elif isinstance(asks[0], (list, tuple)):
                    best_ask = float(asks[0][0])
                    ask_sz = float(asks[0][1]) if len(asks[0]) > 1 else 1.0
                    best_bid = float(bids[0][0])
                    bid_sz = float(bids[0][1]) if len(bids[0]) > 1 else 1.0
                else:
                    continue
                ...
                total_depth = sum(float(a.get("size", a.get("s", 0)) if isinstance(a, dict) else (a[1] if len(a) > 1 else 0)) for a in asks) + \
                              sum(float(b.get("size", b.get("s", 0)) if isinstance(b, dict) else (b[1] if len(b) > 1 else 0)) for b in bids)
                
                snapshots.append({
                    "t": float(row["t_sec"]),
                    "mid": mid, "spread": spread,
                    "imbalance": imbalance, "total_depth": total_depth,
                    "best_ask": best_ask,
                })
            except Exception:
                continue
        ...
        time_span = snapshots[-1]["t"] - snapshots[0]["t"]
        if time_span < 5.0:
            return zeros
        
        # Compute features from snapshots (same logic as clob_features.py)
        imbalances = np.array([s["imbalance"] for s in snapshots])
        spreads = np.array([s["spread"] for s in snapshots])
        mids = np.array([s["mid"] for s in snapshots])
        depths = np.array([s["total_depth"] for s in snapshots])
        t_rel = np.array([s["t"] - snapshots[0]["t"] for s in snapshots])
        ...
        features["clob_spread_trend"] = _slope(t_rel, spreads)
        features["clob_mid_velocity"] = _slope(t_rel, mids)
        mid_diffs = np.diff(mids)
        features["clob_mid_volatility"] = float(np.std(mid_diffs)) if len(mid_diffs) > 0 else 0.0
        features["clob_activity_rate"] = float((len(snapshots) + len(pc_events)) / time_span)
        features["clob_depth_trend"] = _slope(t_rel, depths)
        
        # Ask pressure from price_change events
        ask_prices = []
        for _, row in pc_events.iterrows():
            try:
                side = str(row.get("side", "")).upper()
                if side == "ASK" or "ask" in str(row.get("side", "")).lower():
                    ask_prices.append(float(row.get("price", 0)))
            except:
                continue
        
        if len(ask_prices) >= 2:
            moves_down = sum(1 for i in range(1, len(ask_prices)) if ask_prices[i] < ask_prices[i-1])
            total_moves = sum(1 for i in range(1, len(ask_prices)) if ask_prices[i] != ask_prices[i-1])
            features["clob_ask_pressure"] = float(moves_down / total_moves) if total_moves > 0 else 0.0
        else:
            features["clob_ask_pressure"] = 0.0
        
        return features

    # Group raw OB events by market using slot_ts matching
    # First, we need to map raw OB events to markets
    # The raw parquet likely has a market_id or condition_id column
    log.info("  Columns in raw OB: %s", list(ob_raw.columns))
    
    # Identify market column (varies by dataset)
    market_col = None
    for col_name in ["market_id", "condition_id", "token_id", "asset_id"]:
        if col_name in ob_raw.columns:
            market_col = col_name
            break
    
    if market_col is None:
        log.warning("  Cannot find market_id column in raw OB! Using slot_ts-based matching.")
        # Fallback: match by slot_ts
        ob_raw["slot_ts_val"] = (ob_raw["ts_sec"] // SLOT_DURATION).astype(int) * SLOT_DURATION
        market_col = "slot_ts_val"
    
    clob_features_by_market = {}
    
    # Process in batches per market
    if market_col == "slot_ts_val":
        # Match by slot_ts to market_id
        slot_to_mid = dict(zip(markets["slot_ts"], markets["market_id"]))
        for slot_ts_val, grp in ob_raw.groupby("slot_ts_val"):
            mid = slot_to_mid.get(int(slot_ts_val))
            if mid is None:
                continue
            grp_filtered = grp.copy()
            grp_filtered["t_sec"] = grp_filtered["ts_sec"] - slot_ts_val
            grp_filtered = grp_filtered[(grp_filtered["t_sec"] >= 0) & (grp_filtered["t_sec"] < 60)]
            if len(grp_filtered) >= 2:
                clob_features_by_market[mid] = _compute_clob_features_for_slot(grp_filtered, obs_secs=60)
    else:
        ob_raw["market_id_str"] = ob_raw[market_col].astype(str)
        # Map market_ids from raw OB to our market set
        mid_to_slot = dict(zip(markets["market_id"], markets["slot_ts"]))
        for mid, grp in ob_raw.groupby("market_id_str"):
            if mid not in mid_to_slot:
                continue
            slot_ts = mid_to_slot[mid]
            grp_filtered = grp.copy()
            grp_filtered["t_sec"] = grp_filtered["ts_sec"] - slot_ts
            grp_filtered = grp_filtered[(grp_filtered["t_sec"] >= 0) & (grp_filtered["t_sec"] < 60)]
            if len(grp_filtered) >= 2:
                clob_features_by_market[mid] = _compute_clob_features_for_slot(grp_filtered, obs_secs=60)
    
    log.info("  CLOB features computed for %d / %d markets (%.1f%% coverage)",
             len(clob_features_by_market), len(markets),
             100 * len(clob_features_by_market) / len(markets))
    ...
    for variant_name, obs_secs in VARIANTS_OBS.items():
        ...
        for rank_i, row in markets.iterrows():
            ...
            # ── CLOB real-time microstructure features ─────────────────────
            clob_feat = clob_features_by_market.get(mid)
            if clob_feat is not None:
                feat.update(clob_feat)
            else:
                for cfn in CLOB_FEATURE_NAMES:
                    feat[cfn] = 0.0
    ...
    # ── Save & promote ───────────────────────────────────────────────────
    version_tag = f"v25-{best_variant}"
    ...
    if best_score < 0:  # ALWAYS promote — old champion has formula mismatches with live
        ...
        meta = {
            ...
            "changes": (
                f"v25: CLOB real-time microstructure features (10 new). "
                f"50 features total (40 v23 base + 10 clob_*). "
                f"Trained on raw orderbook events from BrockMisner dataset. "
    ...
    log.info("v25 training complete.")
