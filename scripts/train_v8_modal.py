"""
train_v8_modal.py — BTC 5min model v8

Lessons from v7 permutation importance:
  #1 feature: btc_up_ratio_zscore (anomaly vs last 20 slots) → deepen this
  #2-3: btc_up_w1/w2 (sub-window order flow) → finer granularity (6x30s)
  OB Down token: all zeros (sparse) → dropped. Removed entirely.
  OB Up token bid: proxy for implied prob → keep but as single feature
  Ensemble: didn't help with 601 samples → removed
  Lag features: mostly noise → keep only what survived (lag_2)

New in v8:
  - up_ratio_zscore at 5 / 10 / 20 slots (multi-scale anomaly)
  - per-sub-window zscore: w0/w1/w2 each vs their own history
  - finer tick sub-windows: 6x30s (more temporal resolution)
  - time-weighted order flow: exponential decay toward slot end
  - VWAP trend: is Up vwap rising or falling within slot?
  - volume-weighted momentum (size-weighted, not count-weighted)
  - OB simplified: only Up token bid (implied prob) + spread
  - drop all Down OB features (always sparse/zero)
"""
...
app = modal.App("polymarket-btc-train-v8", image=image)
...
def train_v8():
    ...
    # ── Feature helpers ────────────────────────────────────────────────────────
    def tick_features_v8(grp: pd.DataFrame) -> dict:
        """
        Rich tick features:
        - 6 sub-windows of 30s each (finer than v7's 3x60s)
        - time-weighted order flow (exp decay)
        - VWAP trend within slot
    ...
    for i, row in markets_sorted.iterrows():
        ...
        feat.update(tick_features_v8(grp))
    ...
    model_path = Path("/tmp/btc_model_v8.pkl")
    ...
    if should_promote:
        ...
        api.upload_file(
            ...
            commit_message=(f"Champion v8: AUC={final_auc:.4f} Brier={final_brier:.4f} "
                            f"Acc={final_acc:.4f} | 6x30s windows + multi-scale zscore"),
        )
        meta_out = {
            "version":       "v8",
            ...
            "champion_compared_auc": fair_champ_auc,
            "notes": ("v8: 6x30s sub-windows, time-weighted flow, VWAP trend, "
                      "multi-scale up_ratio zscore (5/10/20), per-window zscores, "
                      "OB Down dropped (sparse), realized vol clustering"),
            "best_params":   final_params,
        }
...
def main():
    print("Submitting BTC v8 training job to Modal...")
    r = train_v8.remote()
    print(f"\n{'='*55}")
